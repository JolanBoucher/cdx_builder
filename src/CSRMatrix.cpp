...
CSRMatrix::EdgeRange CSRMatrix::forward_edges_unchecked(size_t nid) const noexcept {
    ...
    return {
        forward_nodes_.data() + start,
        forward_weights_.data() + start,
        count
    };
}

CSRMatrix::EdgeRange CSRMatrix::backward_edges_unchecked(size_t nid) const noexcept {
    ...
    return {
        backward_nodes_.data() + start,
        backward_weights_.data() + start,
        count
    };
}

// --- Checked Accessors ---
CSRMatrix::Offset CSRMatrix::forward_begin(size_t nid) const {
    check_node_index(nid);
    return forward_key_[nid];
}

CSRMatrix::Offset CSRMatrix::forward_end(size_t nid) const {
    check_node_index(nid);
    return forward_key_[nid + 1];
}

CSRMatrix::Offset CSRMatrix::backward_begin(size_t nid) const {
    check_node_index(nid);
    return backward_key_[nid];
}

CSRMatrix::Offset CSRMatrix::backward_end(size_t nid) const {
    check_node_index(nid);
    return backward_key_[nid + 1];
}

size_t CSRMatrix::forward_degree(size_t nid) const {
    return static_cast<size_t>(forward_end(nid) - forward_begin(nid));
}

size_t CSRMatrix::backward_degree(size_t nid) const {
    return static_cast<size_t>(backward_end(nid) - backward_begin(nid));
}

bool CSRMatrix::has_forward_edges(size_t nid) const {
    return forward_begin(nid) != forward_end(nid);
}

bool CSRMatrix::has_backward_edges(size_t nid) const {
    return backward_begin(nid) != backward_end(nid);
}

CSRMatrix::NodeId CSRMatrix::forward_node_at(Offset position) const {
    check_forward_position(position);
    return forward_nodes_[static_cast<size_t>(position)];
}

CSRMatrix::Weight CSRMatrix::forward_weight_at(Offset position) const {
    check_forward_position(position);
    return forward_weights_[static_cast<size_t>(position)];
}

CSRMatrix::NodeId CSRMatrix::backward_node_at(Offset position) const {
    check_backward_position(position);
    return backward_nodes_[static_cast<size_t>(position)];
}

CSRMatrix::Weight CSRMatrix::backward_weight_at(Offset position) const {
    check_backward_position(position);
    return backward_weights_[static_cast<size_t>(position)];
}

CSRMatrix::Alpha CSRMatrix::forward_alpha(size_t nid) const {
    check_node_index(nid);
    return forward_alpha_[nid];
}

CSRMatrix::Alpha CSRMatrix::backward_alpha(size_t nid) const {
    check_node_index(nid);
    return backward_alpha_[nid];
}

// --- Validation Logic ---
void CSRMatrix::check_node_index(size_t nid) const {
    if (!valid_node_index(nid)) {
        throw std::out_of_range("CSRMatrix: node index " + std::to_string(nid) + " out of bounds");
    }
}

void CSRMatrix::check_forward_position(Offset position) const {
    if (position >= forward_nodes_.size()) {
        throw std::out_of_range("CSRMatrix: forward position out of bounds");
    }
}

void CSRMatrix::check_backward_position(Offset position) const {
...
void CSRMatrix::validate() const {
    ...
    // Parcours unique optimisé par vue
...
uint64_t CSRMatrix::validate_and_sum_forward_view(size_t capacity) const {
    uint64_t sum = 0;
...
uint64_t CSRMatrix::validate_and_sum_backward_view(size_t capacity) const {
    uint64_t sum = 0;
...
void CSRMatrix::validate_alphas(size_t capacity) const {
